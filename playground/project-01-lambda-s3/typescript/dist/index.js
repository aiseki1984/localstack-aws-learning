 ...
 * S3ファイルアップロード
 */
async function uploadFile(key, content, metadata = {}) {
    try {
        const params = {
            Bucket: BUCKET_NAME,
            Key: key,
 ...
 * S3ファイル更新
 */
async function updateFile(key, newContent, metadata = {}) {
    try {
        // 既存ファイルのメタデータを取得
        const headParams = {
            Bucket: BUCKET_NAME,
...
async function runComprehensiveTest() {
    ...
    try {
        ...
        const uploadResult = await uploadFile(testKey, `Hello from TypeScript Lambda!\nTest created at: ${new Date().toISOString()}`, {
        ...
        const updateResult = await updateFile(testKey, `Updated content from TypeScript!\nOriginal: ${readResult.data?.content}`, { 'updated-at': new Date().toISOString() });
    ...
    try {
        ...
        switch (action) {
            case 'test':
                ...
                break;
            case 'upload':
                const uploadRes = await uploadFile(body.key, body.content, body.metadata || {});
                result = {
                    success: uploadRes.success,
                    message: uploadRes.success ? 'File uploaded successfully' : 'Upload failed',
                    data: uploadRes.data,
            ...
            case 'read':
                ...
                break;
            case 'update':
                const updateRes = await updateFile(body.key, body.content, body.metadata || {});
                result = {
                    success: updateRes.success,
                    message: updateRes.success ? 'File updated successfully' : 'Update failed',
                    data: updateRes.data,
            ...
            case 'list':
                ...
                result = {
                    ...
                    message: listRes.success ? 'Files listed successfully' : 'List failed',
                    data: listRes.data,
                    error: listRes.error,
                    language: 'TypeScript',
                    timestamp: new Date().toISOString()
            ...
            case 'delete':
                ...
                result = {
                    ...
                    message: deleteRes.success ? 'File deleted successfully' : 'Delete failed',
                    data: deleteRes.data,
